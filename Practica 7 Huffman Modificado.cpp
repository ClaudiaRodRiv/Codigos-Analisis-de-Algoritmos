...
void printCodes(struct MinHeapNode* root) {
    int arr[MAX_TREE_HT];
    printCodesUtil(root, arr, 0);
}

void encodeText(struct MinHeapNode* root, char* text) {
    ...
    for (int i = 0; i < strlen(text); i++) {
        ...
        if (c != ' ') {
            ...
            while (1) {
                if (temp->left && strchr(temp->left->data, c)) {
                    arr[top] = 0;
                    temp = temp->left;
                } else if (temp->right && strchr(temp->right->data, c)) {
                    arr[top] = 1;
                    temp = temp->right;
                }
                if (isLeaf(temp)) {
